...
g' : forall p' f x z . (forall p x y . p (J y) => p (G (H x y))) =>
                           (forall p x y . p (f (S x) (G (H x z))) => p (f x y)) => 
                               (forall p x y . p (H x (S y)) => p (H (S x) y)) => 
                                   p' (f (S x) (G (H x z)))

g' a3 a1 a2 = a3 (a1 (a2 (g' a3 (\ b1 . a1 (a2 b1)) a2)))
...
applyh a1 
applyh a2 
apply h (\ b1 . [p'0] b1) (\ b1 b2 . [f1] (S b1) b2) (S (S [x2])) (S [z3]) 
-- [p'0] (\ x y . [f1] (S x) y) [x2] (S [z3]) 
intros b1 
applyh a1 
applyh a2 
applyh b1
